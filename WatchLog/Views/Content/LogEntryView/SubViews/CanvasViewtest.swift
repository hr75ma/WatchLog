...
struct CanvasViewtest: UIViewRepresentable {
...
extension PKDrawing {
    mutating func scale(in frame: CGRect) {
        ...
        if bounds.width != frame.width {
            scaleFactor = frame.width / bounds.width
        } else if bounds.height != frame.height {
            scaleFactor = frame.height / bounds.height
        }

        let trasform = CGAffineTransform(scaleX: scaleFactor, y: scaleFactor)

        transform(using: trasform)
    }
}
